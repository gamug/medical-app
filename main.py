...
async def get_available_agenda(data: Agenda=Depends()):
    ...
    Mes_cita = utils.get_ids_equivalence(cursor, data.Mes_cita._value_, 'MES', 'NOMBRE')
    Hospital_atencion = utils.get_ids_equivalence(cursor, data.Hospital_atencion._value_, 'HOSPITAL', 'NOMBRE')
    Sexo_biologico_medico = utils.get_ids_equivalence(cursor, data.Sexo_biologico_medico._value_, 'SEXO_BIOLOGICO', 'SIMBOLO')
    Especialidad_medico = utils.get_ids_equivalence(cursor, data.Especialidad_medico._value_, 'ESPECIALIDAD', 'NOMBRE_ESPECIALIDAD')
    queries = {
